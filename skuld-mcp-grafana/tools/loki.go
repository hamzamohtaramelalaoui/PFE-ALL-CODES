...
const (
	...
	MaxLokiLogLimit = 5000

	maxInfoLinesPerResponse    = 10
	maxDebugLinesPerResponse   = 5
	maxUnknownLinesPerResponse = 5
)

var stackOriginRegex = regexp.MustCompile(`at\s+com\.marketingconfort\.[^\n]+?\(\w+\.java:\d+\)`)
var requestURLRegex = regexp.MustCompile(`https?://[^\s]+`)
var servletContextPathRegex = regexp.MustCompile(`context with path \[([^\]]*)\]`)
var marketingConfortProjectRegex = regexp.MustCompile(`com\.marketingconfort\.([A-Za-z0-9_]+)`)
...
// StructuredLogLine is a normalized log payload derived from a JSON log line.
// It mirrors the Promtail pipeline fields so AI agents receive a stable shape.
...
func (l StructuredLogLine) MarshalJSON() ([]byte, error) {
	payload := make(map[string]any, 6+len(l.Extra))
	if l.Level != "" {
		payload["level"] = l.Level
	}
	if l.Message != "" {
	...
	if l.ErrorType != "" {
		payload["error_type"] = l.ErrorType
	}
	if l.Timestamp != "" {
		payload["timestamp"] = l.Timestamp
	}
	if len(l.Timestamps) > 0 {
		payload["timestamps"] = l.Timestamps
	}
	if l.Occurrences > 0 {
		payload["occurrences"] = l.Occurrences
	}
	if l.ErrorMessage != "" {
		payload["error_message"] = l.ErrorMessage
	}
...
type QueryLokiLogsParams struct {
	...
	Limit         int    `json:"limit,omitempty" jsonschema:"default=10,description=Optionally\\, the maximum number of log lines to return (default max: 5000\\, configurable by MCP server)."`
...
type QueryLokiLogsResult struct {
	Data     []QueryLokiResultItem `json:"data"`
	Hints    *EmptyResultHints     `json:"hints,omitempty"`
	Metadata *QueryMetadata        `json:"metadata,omitempty"`
}

type QueryLokiLine struct {
	Line *StructuredLogLine `json:"line,omitempty"`
}

// QueryLokiResultItem represents either a grouped set of log lines sharing the
// same labels or a metric sample/series.
type QueryLokiResultItem struct {
	Labels map[string]string `json:"labels"`
	Lines  []QueryLokiLine   `json:"lines,omitempty"`
	Value  *float64          `json:"value,omitempty"`
	Values []MetricValue     `json:"values,omitempty"`
}

// LogEntry represents a single parsed log entry or metric sample with metadata
// during normalization before the public response is assembled.
type LogEntry struct {
	Timestamp          string             `json:"timestamp,omitempty"`
	Line               *StructuredLogLine `json:"line,omitempty"`               // For log queries
...
func getNestedString(value any, path ...string) string {
	current := value
...
func getStringByPaths(value map[string]any, paths ...[]string) string {
	for _, path := range paths {
		if len(path) == 0 {
			continue
		}

		// First try exact flat-key lookup like "log.level" or "error.type".
		if len(path) == 1 {
			if v, ok := value[path[0]]; ok {
				switch s := v.(type) {
				case string:
					return s
				case json.Number:
		...
		if s := getNestedString(value, path...); s != "" {
			return s
		}
	}
...
func buildStructuredLogLine(rawLine string) *StructuredLogLine {
	var payload map[string]any
	if err := json.Unmarshal([]byte(rawLine), &payload); err != nil {
		// Preserve non-JSON log lines in a stable object shape rather than
		// returning opaque text.
		return &StructuredLogLine{Message: rawLine}
	}

	line := &StructuredLogLine{
		Timestamp:    getStringByPaths(payload, []string{"@timestamp"}),
		Level:        strings.ToLower(getStringByPaths(payload, []string{"log.level"}, []string{"log", "level"})),
		Message:      getStringByPaths(payload, []string{"message"}),
		ErrorType:    strings.ToLower(getStringByPaths(payload, []string{"error.type"}, []string{"error", "type"})),
		ErrorMessage: getStringByPaths(payload, []string{"error.message"}, []string{"error", "message"}),
	}

	stack := getStringByPaths(payload, []string{"error.stack_trace"}, []string{"error", "stack_trace"})
	var stackOrigin string
	if line.Level == "error" && strings.Contains(line.ErrorType, "exception") && stack != "" {
		stackOrigin = stackOriginRegex.FindString(stack)
	}

	line.Path = extractPath(line.Message, line.ErrorType, stackOrigin, stack)
...
func extractPath(message, errorType, stackOrigin, stack string) string {
	if message == "" {
		if stackOrigin != "" {
			return strings.TrimPrefix(stackOrigin, "at ")
		}
		if project := extractProjectPath(stack); project != "" {
			return project
		}
		return extractProjectPath(errorType)
	}

	if matches := servletContextPathRegex.FindStringSubmatch(message); len(matches) == 2 {
		if matches[1] != "" {
			return matches[1]
		}
	}

	if rawURL := requestURLRegex.FindString(message); rawURL != "" {
...
func extractProjectPath(value string) string {
	if value == "" {
		return ""
	}

	matches := marketingConfortProjectRegex.FindStringSubmatch(value)
	if len(matches) == 2 {
		return matches[1]
	}
...
func renderStructuredLogLine(line *StructuredLogLine) string {
	if line == nil {
		return ""
	}

	// Keep plain text plain when that's all we have.
	if line.Message != "" &&
		line.Timestamp == "" &&
		len(line.Timestamps) == 0 &&
		line.Occurrences == 0 &&
		line.Level == "" &&
		line.ErrorType == "" &&
		line.ErrorMessage == "" &&
...
func parseLokiLogTimestamp(raw json.RawMessage) (string, error) {
	var ts string
	if err := json.Unmarshal(raw, &ts); err == nil {
		return ts, nil
	}
	return "", fmt.Errorf("parsing log timestamp")
}

func formatUnixNanoTimestamp(nano string) string {
	if nano == "" {
		return ""
	}
...
func sanitizeLogLabels(labels map[string]string) map[string]string {
	if len(labels) == 0 {
		return labels
	}
	sanitized := make(map[string]string, len(labels))
	for k, v := range labels {
		if k == "filename" || k == "stream" {
			continue
		}
		sanitized[k] = v
	}
...
func normalizeLogEntry(entry *LogEntry) {
	if entry == nil {
		return
	}

	entry.Labels = sanitizeLogLabels(entry.Labels)

	if entry.Line == nil {
		entry.Line = &StructuredLogLine{}
	}

	if entry.Line.Timestamp == "" {
		entry.Line.Timestamp = formatUnixNanoTimestamp(entry.Timestamp)
	}

	if entry.StructuredMetadata != nil {
		if detectedLevel := strings.ToLower(entry.StructuredMetadata["detected_level"]); detectedLevel != "" {
			if entry.Line.Level == "" {
				entry.Line.Level = detectedLevel
			}
			delete(entry.StructuredMetadata, "detected_level")
		}
		if len(entry.StructuredMetadata) > 0 {
			if entry.Line.Extra == nil {
				entry.Line.Extra = make(map[string]string, len(entry.StructuredMetadata))
			}
			for k, v := range entry.StructuredMetadata {
				entry.Line.Extra[k] = v
			}
		}
		if len(entry.StructuredMetadata) == 0 {
	...
	// For log queries, keep only the normalized timestamp inside line.
	entry.Timestamp = ""
}

func labelsGroupKey(labels map[string]string) string {
	if len(labels) == 0 {
		return ""
	}
...
func groupLogEntriesByLabels(entries []LogEntry) []QueryLokiResultItem {
	if len(entries) == 0 {
		return []QueryLokiResultItem{}
	}

	groups := make([]QueryLokiResultItem, 0)
	groupIndex := make(map[string]int, len(entries))
	for _, entry := range entries {
		key := labelsGroupKey(entry.Labels)
		idx, exists := groupIndex[key]
		if !exists {
			idx = len(groups)
			groupIndex[key] = idx
			groups = append(groups, QueryLokiResultItem{
				Labels: entry.Labels,
				Lines:  []QueryLokiLine{},
			})
		}
		groups[idx].Lines = append(groups[idx].Lines, QueryLokiLine{Line: entry.Line})
	}

	return groups
}

func logDedupKey(line *StructuredLogLine) string {
	if line == nil {
		return ""
	}
	var b strings.Builder
	...
	if line.Level == "error" {
		...
		b.WriteString("\nerror_type=")
		b.WriteString(line.ErrorType)
		b.WriteString("\nerror_message=")
		b.WriteString(line.ErrorMessage)
	}
	return b.String()
}

func deduplicateGroupedLines(groups []QueryLokiResultItem) []QueryLokiResultItem {
	for gi := range groups {
		if len(groups[gi].Lines) == 0 {
			continue
		}

		deduped := make([]QueryLokiLine, 0, len(groups[gi].Lines))
		indexByKey := make(map[string]int, len(groups[gi].Lines))

		for _, groupedLine := range groups[gi].Lines {
			if groupedLine.Line == nil {
				continue
			}

			key := logDedupKey(groupedLine.Line)
			if idx, exists := indexByKey[key]; exists {
				existing := deduped[idx].Line
				existing.Occurrences++
				if groupedLine.Line.Timestamp != "" {
					existing.Timestamps = append(existing.Timestamps, groupedLine.Line.Timestamp)
				}
				existing.Timestamp = ""
				continue
			}

			lineCopy := *groupedLine.Line
			lineCopy.Occurrences = 1
			if lineCopy.Timestamp != "" {
				lineCopy.Timestamps = []string{lineCopy.Timestamp}
				lineCopy.Timestamp = ""
			}

			indexByKey[key] = len(deduped)
			deduped = append(deduped, QueryLokiLine{Line: &lineCopy})
		}

		for i := range deduped {
			deduped[i].Line.Timestamps = summarizeLineTimestamps(deduped[i].Line.Level, deduped[i].Line.Timestamps)
		}

		groups[gi].Lines = deduped
	}

	return groups
}

func classifyLogLevel(level string) string {
...
func applyLogLevelResponseCaps(groups []QueryLokiResultItem) ([]QueryLokiResultItem, bool) {
	if len(groups) == 0 {
		return []QueryLokiResultItem{}, false
	}

	remainingByLevel := map[string]int{
		"info":    maxInfoLinesPerResponse,
		"debug":   maxDebugLinesPerResponse,
		"unknown": maxUnknownLinesPerResponse,
	}

	capped := false
	filteredGroups := make([]QueryLokiResultItem, 0, len(groups))
	for _, group := range groups {
		if len(group.Lines) == 0 {
			continue
		}

		filteredLines := make([]QueryLokiLine, 0, len(group.Lines))
		for _, groupedLine := range group.Lines {
			if groupedLine.Line == nil {
				continue
			}

			levelClass := classifyLogLevel(groupedLine.Line.Level)
			remaining, limited := remainingByLevel[levelClass]
			if limited {
				if remaining <= 0 {
					capped = true
					continue
				}
				remainingByLevel[levelClass] = remaining - 1
			}

			filteredLines = append(filteredLines, groupedLine)
		}
	...
	return filteredGroups, capped
}

func countGroupedLines(groups []QueryLokiResultItem) int {
	total := 0
	for _, group := range groups {
...
func summarizeLineTimestamps(level string, timestamps []string) []string {
	if len(timestamps) <= 1 {
		return timestamps
	}
...
func convertMetricEntries(entries []LogEntry) []QueryLokiResultItem {
	if len(entries) == 0 {
		return []QueryLokiResultItem{}
	}

	items := make([]QueryLokiResultItem, 0, len(entries))
	for _, entry := range entries {
		items = append(items, QueryLokiResultItem{
			Labels: entry.Labels,
			Value:  entry.Value,
			Values: entry.Values,
		})
...
func queryLokiLogs(ctx context.Context, args QueryLokiLogsParams) (*QueryLokiLogsResult, error) {
			...
			for _, value := range stream.Values {
				if len(value) >= 2 {
					...
					entry := LogEntry{
						...
						Line:      buildStructuredLogLine(logLine),
					...
					logTS, err := parseLokiLogTimestamp(value[0])
					if err == nil {
						entry.Timestamp = logTS
					}
	...
	if response.Data.ResultType == "streams" {
		clusterer := newDrain3TemplateClient(cfg)
		if clusterer != nil {
			clusteredEntries, clusterErr := applyTemplateClustering(ctx, entries, clusterer)
			if clusterErr != nil {
				slog.Warn("failed to cluster low-signal Loki logs", "error", clusterErr)
			} else {
	...
	// Build the response
	var data []QueryLokiResultItem
	linesReturned := len(entries)
	if response.Data.ResultType == "streams" {
		data = deduplicateGroupedLines(groupLogEntriesByLabels(entries))
		linesReturned = countGroupedLines(data)
	} else {
		data = convertMetricEntries(entries)
	}
