...
func searchLogsInLoki(ctx context.Context, args SearchLogsParams, limit int, startRFC3339, endRFC3339 string) (*SearchLogsResult, error) {
	...
	for _, group := range entries.Data {
		for _, groupedLine := range group.Lines {
			if groupedLine.Line == nil {
				continue
			}
			timestamp := groupedLine.Line.Timestamp
			if timestamp == "" && len(groupedLine.Line.Timestamps) > 0 {
				timestamp = groupedLine.Line.Timestamps[0]
			}
			logs = append(logs, LogResult{
				Timestamp: timestamp,
				Message:   renderStructuredLogLine(groupedLine.Line),
				Labels:    group.Labels,
			})
		}
	}
