...
- `--loki-drain3-url`: Optional HTTP URL for a Drain3 clustering service. When set, `query_loki_logs` clusters `info`, `debug`, and unknown log messages into templates before deduplication to reduce noisy output.
- `--loki-drain3-timeout`: Timeout for requests to the optional Drain3 clustering service - default: `3s`
...
### Optional Drain3 Sidecar For Loki

If your Loki logs contain lots of repetitive `info` or `debug` messages, you can run the included Drain3 sidecar to compress those messages into templates before `query_loki_logs` groups and deduplicates them.

1. Install the Python dependency with `pip install -r drain3/requirements.txt`
2. Start the sidecar with `python drain3/service.py`
3. Start `mcp-grafana` with `--loki-drain3-url http://127.0.0.1:8011`

Errors and warnings keep their existing richer structure. The Drain3 sidecar only rewrites lower-signal messages so `query_loki_logs` can return fewer characters without losing the overall pattern of what happened.
